...
export function classifyOcrQuality(boxes: TextBox[]): OcrQuality {
  ...
  if (boxes.length === 0) {
    return quality(
      ...
      0,
      false,
      "Không tìm thấy chữ trong ảnh. Vui lòng chụp lại hoặc nhập tay.",
    )
  }
  ...
  if (text.length < MIN_TEXT_LENGTH) {
    return quality(
      ...
      averageConfidence,
      false,
      "Không đọc được đơn thuốc, vui lòng nhập tay.",
    )
  }
  ...
  if (averageConfidence < LOW_CONFIDENCE) {
    return quality(
      ...
      averageConfidence,
      true,
      "Ảnh đơn thuốc chưa rõ. Vui lòng kiểm tra lại kết quả trước khi lưu.",
    )
  }
